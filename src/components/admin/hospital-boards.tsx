...
import { BoardImageGallery, SafePrizeImage } from "@/components/board-image-gallery";
...
export function HospitalBoards() {
  ...
  return <>
    ...
    <Card><SectionTitle title={`${selected.boardName} 상품 현황`} sub={`공개 상품 ${selected.prizes.length}종 · 잔여 수량과 프로그램 이미지`}/><div className="admin-prize-grid">{selected.prizes.map((prize, index) => <article key={prize.id}><span>{String.fromCharCode(65 + Math.min(index, 25))}상</span><SafePrizeImage src={prize.image} alt={prize.name}/><b>{prize.name}</b><Badge tone={prize.available ? "green" : "gray"}>{prize.available ? `${prize.remainingCount ?? ""}개 남음` : "소진"}</Badge></article>)}{!selected.prizes.length && <div className="admin-empty"><Ticket/><p>공개된 상품 정보가 없습니다.</p></div>}</div></Card>
    ...
    <Card><BoardImageGallery boards={hospital.boards} selectedId={selected.id} onSelect={setSelectedId}/></Card>
  </>;
}
