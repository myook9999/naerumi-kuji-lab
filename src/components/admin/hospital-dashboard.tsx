...
import { Activity, CircleCheck, CircleDollarSign, Clock3, HeartPulse, Receipt, ShieldCheck, Ticket, Users } from "lucide-react";
...
export function HospitalDashboard() {
  ...
  return <>
    ...
    <div className="kpi-grid six hospital-kpis">
      <KpiCard label="현재 예상 매출" value={won(sales.grossSales)} note="공개판 판매 기준" icon={CircleDollarSign}/>
      <KpiCard label="예상 정산액" value={won(sales.estimatedNet)} note="임시 수수료 3.5%" icon={Receipt} tone="mint"/>
      ...
      <KpiCard label="강화 치료 중" value={`${approved.filter((member) => member.treatmentStage > 0 && member.treatmentStage < MAX_TREATMENT_STAGE).length}명`} note="최대 15강 치료" icon={HeartPulse} tone="pink"/>
    </div>
    ...
    <Card><SectionTitle title="환자 강화 치료 현황" sub="승인 환자의 0~15강 진행 상태를 확인합니다" action={<Link href="/admin/treatment">치료 현황 보기</Link>}/><div className="treatment-overview">{approved.slice(0, 6).map((member) => <article key={member.uid}><span>{member.name.slice(0, 1)}</span><div><b>{member.name}</b><small>{treatmentStages[Math.min(MAX_TREATMENT_STAGE, member.treatmentStage)].name}</small></div><Progress value={member.treatmentStage / MAX_TREATMENT_STAGE * 100} color="#C95F5C"/><strong>{member.points.toLocaleString()}P</strong></article>)}</div></Card>
  </>;
}
