...
export function HospitalStorefront() {
  ...
  return <main className="patient-app">
    ...
    <section id="chart" className="patient-section"><div className="hospital-section-title"><span>LIVE KUJI CHART</span><h2>실시간 쿠지 차트</h2><p>쿠지병동 프로그램에서 필요한 공개 정보만 읽어옵니다.</p></div>
      ...
      <Card className="patient-board-results"><div className="board-result-title"><div><span>BOARD DRAW RESULT</span><h3>{hospital.board.boardName} 고객별 뽑기 결과</h3><p>닉네임별 상위상과 랜덤굿즈 수량을 집계했습니다. 계정·주소·당첨번호는 공개하지 않습니다.</p></div><Badge tone="red">{hospital.board.customerResults?.length ?? 0}명</Badge></div><div className="board-result-list">{hospital.board.customerResults?.map((customer) => <article key={customer.id}><div className="result-customer"><span>{customer.nickname.slice(0, 1)}</span><div><b>{customer.nickname}</b><small>총 {customer.totalDraws.toLocaleString()}회 뽑기</small></div></div><strong>랜덤굿즈 {customer.randomGoodsCount.toLocaleString()}개</strong><div className="result-upper-prizes">{customer.upperPrizes.length ? customer.upperPrizes.map((prize) => <Badge key={prize.name} tone="red">{prize.name}{prize.count > 1 ? ` ×${prize.count}` : ""}</Badge>) : <Badge tone="gray">상위상 없음</Badge>}</div></article>)}</div></Card>
    </section>

    {hospital.boardVisible && <Card className="patient-board-images"><BoardImageGallery boards={hospital.boards} selectedId={hospital.board.id} onSelect={hospital.selectBoard} title="전체 쿠지판 상품 이미지"/></Card>}
    <PatientFulfillment/>
