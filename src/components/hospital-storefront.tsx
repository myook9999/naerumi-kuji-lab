...
import { CircleCheck, Clock3, HeartPulse, LogOut, Sparkles, Ticket, UserRoundCheck } from "lucide-react";
...
export function HospitalStorefront() {
  ...
  const stage = treatmentStages[session.treatmentStage];
  const nextStage = treatmentStages[Math.min(5, session.treatmentStage + 1)];
  ...
  async function treat() {
    ...
    try {
      ...
      const message = outcome.success ? `${treatmentStages[outcome.afterStage].name} 단계 치료에 성공했어요!` : "치료 효과가 조금 부족했어요. 현재 단계는 안전하게 유지됩니다.";
      setResult(message);
  ...
  return <main className="patient-app">
    ...
    <section id="chart" className="patient-section"><div className="hospital-section-title"><span>LIVE KUJI CHART</span><h2>실시간 쿠지 차트</h2><p>쿠지병동 프로그램에서 필요한 공개 정보만 읽어옵니다.</p></div>
      {!hospital.boardVisible ? <Card className="board-hidden"><Clock3/><h3>현재 쿠지판 진료가 잠시 중단됐어요</h3><p>사장님이 다시 공개하면 자동으로 표시됩니다.</p></Card> : <Card className="patient-board-card"><div className="board-visual"><Image src={assets.ticket} alt="쿠지 티켓" width={190} height={190}/><Badge tone={hospital.boardConnection === "live" ? "green" : "amber"}>{hospital.boardConnection === "live" ? "실시간 연동" : hospital.boardConnection === "demo" ? "시연 데이터" : "연결 확인 중"}</Badge></div><div className="board-chart-content"><span>현재 진료 중</span><h3>{hospital.board.boardName}</h3><p>{hospital.board.price || "가격 정보 확인 중"} · 총 {hospital.board.totalCards}장</p><div className="chart-numbers"><strong>{hospital.board.remainingCards}<small>장 남음</small></strong><span>{hospital.board.openedCount}장 처방 완료</span></div><Progress value={progress} color="#C95F5C"/><footer><span>마지막 반영</span><b>{hospital.board.updatedAt ? new Date(hospital.board.updatedAt).toLocaleString("ko-KR") : "확인 중"}</b></footer>{hospital.boardError && <p className="inline-error">{hospital.boardError}</p>}</div></Card>}
    ...
    <section id="treatment" className="patient-section treatment-section"><div className="hospital-section-title"><span>POINT TREATMENT</span><h2>포인트 치료실</h2><p>실패해도 치료 단계는 내려가지 않으며 사용 포인트와 결과가 기록됩니다.</p></div><div className="treatment-grid"><Card className="treatment-machine"><div className="treatment-orb"><HeartPulse/><small>CURRENT CARE</small><b>{stage.name}</b><span>{stage.description}</span></div><div className="treatment-metrics"><span><small>다음 진료</small><b>{session.treatmentStage >= 5 ? "완치" : nextStage.name}</b></span><span><small>필요 포인트</small><b>{session.treatmentStage >= 5 ? "-" : `${nextStage.cost.toLocaleString()}P`}</b></span><span><small>치료 성공률</small><b>{session.treatmentStage >= 5 ? "100%" : `${nextStage.probability}%`}</b></span></div>{result && <p className="treatment-result"><Sparkles/>{result}</p>}<Button onClick={treat} disabled={treating || session.treatmentStage >= 5 || session.points < nextStage.cost}>{treating ? "치료 기록 중..." : session.treatmentStage >= 5 ? "완치 판정 완료" : `${nextStage.name} 치료받기`}</Button></Card><Card className="care-path"><h3>나의 치료 경로</h3>{treatmentStages.map((item, index) => <div key={item.name} className={index <= session.treatmentStage ? "done" : index === session.treatmentStage + 1 ? "next" : ""}><i>{index < session.treatmentStage ? <CircleCheck/> : index + 1}</i><span><b>{item.name}</b><small>{item.description}</small></span>{index === session.treatmentStage && <Badge tone="red">현재</Badge>}</div>)}</Card></div></section>
